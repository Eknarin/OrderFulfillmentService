...
public class OrderResource {
	...
	private String paymentService = "http://128.199.212.108:8000";
	...
	public Response getOrders(@HeaderParam("Accept") String accept,  @Context javax.ws.rs.core.Request request) {
	...
	@Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
	public Response getOrderById(@PathParam("id") long id, @HeaderParam("Accept") String accept, @Context javax.ws.rs.core.Request request) {
	...
	public Response checkOrderShipmentCost(String order, @HeaderParam("Content-Type") String type,
			@HeaderParam("Accept") String accept, @Context javax.ws.rs.core.Request request){
		
		logger.info("POST COST type = " + type);
		Request req = client.newRequest(shipmentService + "/shipments/calculate");
		Order o ;
	...
	public Response checkPayment(String order, @HeaderParam("Content-Type") String type){
		...
		String paymentJSON = toJson(payment,Payment.class);
		System.out.println(paymentJSON);
		
		StringContentProvider content = new StringContentProvider(paymentJSON);
		request.method(HttpMethod.POST);
		request.content(content, MediaType.APPLICATION_JSON);
		...
		logger.info("pass" + res.getStatus());
	...
	public Response getOrderPaymentStatus(@PathParam("id") long id, @HeaderParam("Accept") String accept,
		...
		Order order = dao.find(id);
		logger.info("GET STATUS id = " + id + "accept + " + accept);
		...
		boolean success = res.getContentAsString().contains(Payment.SUCCESS);
		logger.info("status : " + res.getContentAsString() + " " + success );
		if(success){
	...
	public Order createOrder(Order order,long paymentID, String location){		
		...
		o.setMerchant_email(order.getMerchant_email());
	 ...
	 @RolesAllowed({"admin","e-commerce"})
	 @Path("{id}/cancel")
	 public Response cancelOrder(@PathParam("id") long id, @Context javax.ws.rs.core.Request request){
	 ...
	 @RolesAllowed({"admin","fulfiller"})
	 @Path("{id}/grab")
	 public Response grabOrder(@PathParam("id") long id, @Context javax.ws.rs.core.Request request){
	 ...
	 @RolesAllowed({"admin","fulfiller"})
	 @Path("{id}/fulfill") 
	 public Response fulfillOrder(@PathParam("id") long id, @Context javax.ws.rs.core.Request request){
	 ...
	 @RolesAllowed({"admin","fulfiller"})
	 @Path("{id}/undo") 
	 public Response undoOrder(@PathParam("id") long id, @Context javax.ws.rs.core.Request request){
		 logger.info("PUT UNDO id = " + id);
		 
		 Order o = dao.find(id);		
		 if(o != null){
			 String status = o.getStatus();
			 if(!status.equals(Order.PENDING_PAYMENT)){
				 if(status.equals(Order.IN_PROGRESS)){
					 o.setStatus(Order.WAITING);
				 }
				 else if(status.equals(Order.WAITING)){
					 o.setStatus(Order.PENDING_PAYMENT);
				 }
	 ...
	 @RolesAllowed({"admin","fulfiller"})
	 @Path("{id}/ship") 
	 public Response shipOrder(@PathParam("id") long id, @Context javax.ws.rs.core.Request request){
	 ...
	 /**
	  * Convert xml string to payment object.
	  * @param s xml string
	  * @return converted payment
	  */
	 private Payment stringXMLtoPayment(String s){
		 JAXBContext context;
		 try {
			 StringReader sr = new StringReader(s);
			 context = JAXBContext.newInstance(Payment.class);
			 Unmarshaller unmarshaller = context.createUnmarshaller();
			 Payment payment = (Payment) unmarshaller.unmarshal(sr);
			 return payment;
		 } catch (JAXBException e) {
			 logger.info("Cannot convert String XML to Payment.");
		 }	 
		 return null;
	 }
	  ...
	  * @return converted order.
	  */
	 private Payment stringJSONtoPayment(String s){
		 JSONObject json = new JSONObject(s);
		 String xml = XML.toString(json);
		 return stringXMLtoPayment(xml);
	 }
	 ...
	 public Response returnWithETagPUT(Object ob, javax.ws.rs.core.Request request){
		 ...
		 Response.ResponseBuilder builder = request.evaluatePreconditions(etag);
		 if (builder == null){
			 builder = Response.ok(uriInfo.getAbsolutePath()+"");
		 }
		 builder.cacheControl(cache);
